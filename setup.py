        ...
        hooks[event] = [
            ...
            if not any(
                any(marker in (handler.get('command', '') or '')
                    for marker in ['mission_control', '.claude/hooks/minion_orchestra', 'minion_orchestra_hook.py'])
...
def setup():
    ...
    for event in HOOK_EVENTS:
        ...
        already = any(
            any('claude_hook.py' in (handler.get('command', '') or '')
                for handler in group.get('hooks', []))
        ...
        if not already:
            ...
            for group in settings['hooks'][event]:
                for handler in group.get('hooks', []):
                    if 'claude_hook.py' in (handler.get('command', '') or ''):
