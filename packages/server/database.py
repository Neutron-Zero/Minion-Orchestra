...
async def store_transcript_entries_batch(entries: list[dict[str, Any]]) -> int:
    ...
    await db.executemany(
        ...
        INSERT INTO transcript_entries
            (agent_id, session_id, entry_type, content, tool_name, tool_input, tool_use_id, timestamp, line_number, metadata)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        [
            (
                e["agent_id"], e["session_id"], e["entry_type"], e.get("content"),
...
async def get_transcript_entries(
    ...
    """Query the most recent transcript entries for an agent, returned in chronological order."""
    db = _get_db()
    cursor = await db.execute(
...
async def upsert_transcript_scan_state(
    ...
    await db.execute(
        """
        INSERT INTO transcript_scan_state (agent_id, session_id, jsonl_path, last_line_number, last_file_size)
        VALUES (?, ?, ?, ?, ?)
        ...
        """,
        (agent_id, session_id, jsonl_path, last_line_number, last_file_size),
    )
    await db.commit()
