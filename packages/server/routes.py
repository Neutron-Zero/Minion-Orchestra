...
from services import agent_manager, task_queue, log_hook_data, read_hook_logs, clear_hook_logs
...
def _parse_ts(timestamp: str | None) -> datetime | None:
    """Parse an ISO timestamp string into a datetime, or return None."""
    if not timestamp:
        return None
    try:
...
def _safe_isoformat(value) -> str:
    """Return isoformat string whether value is datetime or already a string."""
...
async def _handle_subagent_start(agent, socket_id, event, sio):
    ...
    await db.store_session(
        agent_id=subagent_id, agent_name=subagent.name, status="working",
...
async def _handle_subagent_stop(agent, socket_id, event, sio):
    ...
    for sub in subagents:
        ...
        await db.store_session(
            agent_id=sub.id, agent_name=sub.name, status="completed",
            working_directory=sub.working_directory,
            start_time=_safe_isoformat(sub.start_time),
            pid=sub.pid,
...
async def _handle_permission_request(agent, socket_id, event, sio):
    ...
    # Build a short summary for the card/detail display
    summary = tool_name
    if isinstance(tool_input, dict):
        desc = tool_input.get("description") or tool_input.get("command") or tool_input.get("file_path") or ""
        if desc:
            summary = f"{tool_name}: {desc[:120]}"
    agent.current_task = summary
...
async def _handle_session_end(agent, socket_id, event, sio):
    ...
    # Copilot fires sessionEnd after each turn, not when the session is truly
    # over.  Treat it like Claude's Stop (idle) rather than offline.
    if agent.type == "copilot-cli":
        agent.status = "idle"
        agent.current_task = None
        agent.current_tool = None
        await _emit_and_store_log(sio, event.timestamp, "info", f"Turn finished ({reason})", event.agentId)
    else:
...
async def hook_endpoint(event: HookEvent, request: Request):
    try:
        ...
        if not agent:
            # Determine agent type from source_tool or parentPid
            source_tool = (event.data or {}).get("source_tool")
            if source_tool:
                agent_type = source_tool
            else:
            ...
            default_name = "Copilot Agent" if source_tool == "copilot-cli" else "Claude Agent"
            agent = agent_manager.create_agent(
                id=event.agentId, socket_id=f"hook-{event.agentId}",
                name=event.agentName or default_name, type=agent_type, status="idle",
            ...
            if event.pid and (not agent.pid or agent.type == "copilot-cli"):
                agent.pid = event.pid
        ...
        # Broadcast structured event for Event Stream view
        ...
        await db.store_session(
            agent_id=event.agentId, agent_name=agent.name, status=agent.status,
            working_directory=agent.working_directory,
            start_time=_safe_isoformat(agent.start_time) if agent.start_time else _ts(event.timestamp),
        ...
        # Copilot doesn't need working_directory to resolve its JSONL path
        can_scan = session_id and (agent.working_directory or source_tool == "copilot-cli")
        if can_scan:
            try:
                scanner = request.app.state.transcript_scanner
                scanner.track_agent(event.agentId, session_id, agent.working_directory, source_tool)
                await scanner.scan_agent(event.agentId, session_id, agent.working_directory, source_tool)
            except Exception as scan_err:
                print(f"Transcript scan error for {event.agentId}: {scan_err}")
...
@router.get("/api/images/{entry_id}")
async def get_transcript_image(entry_id: str):
    """Serve an image stored in a transcript entry's metadata."""
    try:
        entry = await db.get_transcript_entry_by_id(entry_id)
        if not entry:
            return Response(status_code=404, content="Entry not found")

        metadata = entry.get("metadata")
        if not metadata or not isinstance(metadata, dict):
            return Response(status_code=404, content="No image data")

        image_data = metadata.get("image_data")
        image_type = metadata.get("image_type", "png")
        if not image_data:
            return Response(status_code=404, content="No image data")

        raw_compressed = base64.b64decode(image_data)
        raw_image = gzip.decompress(raw_compressed)
...
async def patch_config(request: Request):
    try:
        body: dict[str, Any] = await request.json()
        for section in ("session_watcher", "terminal"):
...
async def get_events_endpoint(
    ...
    try:
        events = await db.get_events(
            limit=limit + offset, event_type=event_type,
            agent_id=agent_id, since=since,
        )
        if search:
            term = search.lower()
            events = [e for e in events if
                      term in (e.get("message", "") or "").lower() or
                      term in (e.get("event_type", "") or "").lower() or
                      term in json.dumps(e.get("metadata", {}) or {}, default=str).lower()]
...
async def get_agent_transcript(
    agent_id: str, limit: int = 500, offset: int = 0,
    start_line: int | None = None, before_line: int | None = None,
    anchor_timestamp: str | None = None,
):
    try:
        # If anchor_timestamp provided, resolve to a line_number first
        if anchor_timestamp and start_line is None:
            resolved = await db.get_transcript_line_for_timestamp(agent_id, anchor_timestamp)
            if resolved is not None:
                # Load 100 before the target, 900 after
                start_line = max(1, resolved - 100)

        total = await db.get_transcript_count(agent_id)

        if before_line is not None:
            entries = await db.get_transcript_entries_before(
                agent_id=agent_id, before_line=before_line, limit=limit,
            )
        else:
            entries = await db.get_transcript_entries(
        ...
        return {
            ...
            "total": total, "first_line": first_line, "last_line": last_line,
        }
    except Exception as e:
        return {"success": False, "error": str(e)}
    ...
    try:
        ...
        if search:
            ...
            sessions = [s for s in sessions if
                        ...
                        term in (s.get("agent_id", "") or "").lower()]
        if cwd:
