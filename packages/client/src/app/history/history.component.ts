...
export class HistoryComponent implements OnInit {
  ...
  limit = 50;
  hasMore = true;
  private expandedParents = new Set<string>();
  ...
  isSubagent(session: Session): boolean {
    ...
    if (session.agent_id && session.agent_id.includes('-sub-')) return true;
    return false;
  }
  ...
  toggleChildren(row: DisplayRow, event: Event): void {
    event.stopPropagation();
    if (row.expanded) {
      this.expandedParents.delete(row.session.agent_id);
    } else {
      this.expandedParents.add(row.session.agent_id);
    }
    this.buildDisplayRows();
    this.cdr.markForCheck();
  }
  ...
  private buildDisplayRows(): void {
    // Collect all non-subagent sessions by PID (multiple sessions may share a PID over time)
    const pidToParents = new Map<number, Session[]>();
    for (const s of this.sessions) {
      if (s.pid && !this.isSubagent(s)) {
        const list = pidToParents.get(s.pid) || [];
        list.push(s);
        pidToParents.set(s.pid, list);
      }
    }

    // Group children by parent session id
    const childrenByParentId = new Map<string, Session[]>();
    const topLevel: Session[] = [];

    for (const s of this.sessions) {
      if (this.isSubagent(s)) {
        const parentPid = s.metadata?.['parent_pid'];
        const candidates = parentPid ? pidToParents.get(parentPid) : undefined;
        const parent = candidates?.find(p => this.timeOverlaps(p, s));
        if (parent) {
          const list = childrenByParentId.get(parent.agent_id) || [];
          list.push(s);
          childrenByParentId.set(parent.agent_id, list);
          continue;
        }
      }
      topLevel.push(s);
    ...
    for (const s of topLevel) {
      const children = childrenByParentId.get(s.agent_id) || [];
      const expanded = this.expandedParents.has(s.agent_id);
      rows.push({ session: s, isChild: false, childCount: children.length, expanded });
      if (expanded) {
        for (const child of children) {
          rows.push({ session: child, isChild: true, childCount: 0, expanded: false });
        }
      }
    }
  ...
  private timeOverlaps(parent: Session, child: Session): boolean {
    if (!parent.start_time || !child.start_time) return false;
    const parentStart = new Date(parent.start_time).getTime();
    const childStart = new Date(child.start_time).getTime();
    if (isNaN(parentStart) || isNaN(childStart)) return false;
  ...
  trackByRow(index: number, row: DisplayRow): string {
    return row.session.agent_id;
  }
