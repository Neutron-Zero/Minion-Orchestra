...
export class KanbanBoardComponent {
  ...
  @Output() approve = new EventEmitter<Agent>();
  @Output() deny = new EventEmitter<Agent>();
  @Output() sendInput = new EventEmitter<{ agent: Agent; text: string }>();
  ...
  onApprove(agent: Agent): void {
    this.approve.emit(agent);
  }

  onDeny(agent: Agent): void {
  ...
  onSendInput(agent: Agent, event: Event): void {
    ...
    if (!card) return;
    const input = card.querySelector('.card-freeform') as HTMLInputElement;
    if (!input) return;
    const text = input.value.trim();
    if (!text) return;
    this.sendInput.emit({ agent, text });
    input.value = '';
  }
